...
function CompletionBanner({ label, onReset, color }: { label: string; onReset: () => void; color: string }) {
  return (
    <div style={{ background:color, borderRadius:12, padding:'16px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', margin:'12px 24px', gap:12 }}>
      ...
      <button onClick={onReset} style={{ background:'transparent', border:'1px solid rgba(255,255,255,0.3)', color:'#fff', fontSize:11, fontWeight:600, padding:'6px 14px', borderRadius:8, cursor:'pointer' }}>Reset</button>
    </div>
  )
}
...
function CheckItem({ emoji, label, sub, link, checked, onToggle, outlineBtn, brandColor }: { emoji?:string; label:string; sub?:string; link?:{href:string;text:string}; checked:boolean; onToggle:()=>void; outlineBtn?:boolean; brandColor:string }) {
  return (
    <li style={{ display:'flex', alignItems:'center', gap:14, padding:'18px 24px', borderBottom:'1px solid #F0EBE1' }}>
      <div onClick={onToggle} style={{ width:28, height:28, borderRadius:'50%', flexShrink:0, border:`2px solid ${checked ? brandColor : '#D1C9B8'}`, background: checked ? brandColor : 'transparent', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:13, fontWeight:700, transition:'all 0.2s', cursor:'pointer' }}>
        {checked ? '✓' : ''}
      </div>
      ...
      {link && (
        <a href={link.href} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()}
          style={outlineBtn
            ...
            : { fontSize:13, fontWeight:600, color:'#fff', background:brandColor, padding:'8px 18px', borderRadius:8, textDecoration:'none', whiteSpace:'nowrap', flexShrink:0 }
          }>
          {link.text}
        </a>
      )}
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:'#F5F1EA', minHeight:'100vh', color:'#2C2416' }}>
      ...
      {isAdmin && (
        <div style={{ background:primary, padding:'10px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
          ...
          <a href={`/admin/${org.slug}`}
            style={{ fontSize:12, fontWeight:600, color:'#fff', background:accent, padding:'6px 18px', borderRadius:8, textDecoration:'none' }}>
            ← Back to Admin Panel
          </a>
        </div>
      )}
      ...
      <nav style={{ background:primary, display:'flex', justifyContent:'center', gap:8, padding:'14px 20px', flexWrap:'wrap' }}>
        {[['#mm-daily','Daily Videos'],['#mm-guides','Guides'],['#mm-tips','Tips'],['#mm-referral','Referral']].map(([href,label])=>(
          <a key={href} href={href} style={{ padding:'8px 20px', fontSize:13, fontWeight:500, textDecoration:'none', color:'#fff', borderRadius:8, border:'1px solid rgba(255,255,255,0.25)', background:'transparent', whiteSpace:'nowrap' }}>{label}</a>
        ))}
      </nav>
      ...
      <header style={{ background:`linear-gradient(180deg, ${primary} 0%, ${primary}dd 100%)`, padding:'48px 24px 0', textAlign:'center', position:'relative' }}>
        ...
        <h1 style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'clamp(42px,10vw,72px)', fontWeight:700, fontStyle:'italic', color:'#FDFAF4', margin:'0 0 16px', lineHeight:1.05 }}>{org.name}</h1>
        <p style={{ color:'rgba(255,255,255,0.65)', fontSize:15, maxWidth:560, margin:'0 auto 36px', lineHeight:1.7 }}>
        ...
        <a href="https://vimeo.com/835524024?share=copy" target="_blank" rel="noreferrer"
          ...
          <div style={{ width:48, height:48, borderRadius:'50%', background:primary, display:'flex', alignItems:'center', justifyContent:'center', fontSize:18, color:'white', flexShrink:0 }}>▶</div>
      ...
      <main style={{ maxWidth:780, margin:'0 auto', padding:'56px 20px 80px' }}>
        ...
        <div style={{ background:'#fff', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', marginBottom:16 }}>
          ...
          {!onboard.loading && <>
            {onboard.allDone && <CompletionBanner label="Onboarding complete! You're ready to go." onReset={onboard.reset} color={primary} />}
            <ul style={{ listStyle:'none', padding:0, margin:0 }}>
              {onboardItems.map((item,i)=>(
                <CheckItem key={i} label={item.label} sub={item.sub} link={item.link} checked={onboard.checked[i]} onToggle={()=>onboard.toggle(i)} brandColor={primary} />
              ))}
            </ul>
          </>}
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', marginBottom:16 }}>
          ...
          {!essentials.loading && <>
            {essentials.allDone && <CompletionBanner label="You've got everything you need!" onReset={essentials.reset} color={primary} />}
            <ul style={{ listStyle:'none', padding:0, margin:0 }}>
              {essItems.map((item,i)=>(
                <CheckItem key={i} emoji={item.emoji} label={item.label} sub={item.sub} link={item.link} checked={essentials.checked[i]} onToggle={()=>essentials.toggle(i)} outlineBtn brandColor={primary} />
              ))}
            </ul>
          </>}
        </div>
        ...
        <div style={{ background:'#fff', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', marginBottom:16 }}>
          ...
          <div style={{ padding:'0 28px 28px', display:'flex', flexDirection:'column', gap:10 }}>
            <a href={zoomLink} target="_blank" rel="noreferrer" style={{ background:primary, color:'#fff', fontSize:15, fontWeight:600, padding:'14px 0', borderRadius:8, textDecoration:'none', textAlign:'center', display:'block' }}>Join Zoom</a>
        ...
        <a href="https://hohdigitallibrary.com/nightbefore" target="_blank" rel="noreferrer"
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontWeight:700, fontSize:17, color:'#fff', marginBottom:4 }}>Night Before You Begin</div>
            <div style={{ fontSize:13, color:'rgba(255,255,255,0.5)' }}>Prep your environment. Set expectations. Start clean.</div>
          </div>
          ...
          <ol style={{ listStyle:'none', padding:0, margin:0 }}>
            {[{t:'Log in at optavia.com',b:'Open your Account Dashboard. Temp password: Welcome1! (capital W + !)'},
              ...
              <li key={i} style={{ display:'flex', gap:14, padding:'12px 0', borderBottom:i<3?'1px solid #F0EBE1':'none', fontSize:14 }}>
                <span style={{ width:26, height:26, background:primary, color:'white', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, fontWeight:700, flexShrink:0 }}>{i+1}</span>
