...
export async function sendPasswordResetEmail({
  ...
  await getResend().emails.send({
    from:    FROM,
    to,
    subject: 'Reset your password — The Daily Reset',
    html: `
      <div style="font-family: 'DM Sans', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 24px; background: #F7F2E8;">
        <h1 style="font-size: 28px; color: #2C2416; margin-bottom: 8px;">The Daily Reset</h1>
        ...
        <p style="color: #A89E8C; font-size: 11px;">© The Daily Reset · All rights reserved</p>
      </div>
    `,
  })
}
...
export async function sendCoachInviteEmail({
  ...
  await getResend().emails.send({
    from:    FROM,
    to,
    subject: `You're invited to join ${orgName} — The Daily Reset`,
    html: `
      <div style="font-family: 'DM Sans', sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 24px; background: #F7F2E8;">
        <h1 style="font-size: 28px; color: #2C2416; margin-bottom: 8px;">The Daily Reset</h1>
        ...
        <p style="color: #7A6E5C; font-size: 14px; line-height: 1.6; margin-bottom: 8px;">
          <strong>${invitedBy}</strong> has invited you to join <strong>${orgName}</strong> as a coach admin on The Daily Reset platform.
        </p>
        <p style="color: #7A6E5C; font-size: 14px; line-height: 1.6; margin-bottom: 28px;">
